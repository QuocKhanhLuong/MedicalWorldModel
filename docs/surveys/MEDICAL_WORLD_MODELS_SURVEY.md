...
Thông tin ngăn chọn topic hôm nay: audit metadata mới đóng một phần số lượng/cấu trúc; chưa biết số bệnh nhân đủ history, future, nhãn và khoảng thời gian phù hợp cùng lúc; chưa có nhãn lỗi/uncertainty được định lượng; chưa chốt downstream endpoint/horizon với chuyên gia; chưa xác nhận command/contact hoặc treatment-time completeness; chưa đo baseline headroom. Bước tiếp theo là kiểm tra dữ liệu và giao thức như [DATASET_FEASIBILITY](../DATASET_FEASIBILITY.md), **chưa chọn backbone**.

## Đợt 2 — thay đổi sau khi đọc sâu methods/evaluation

| VERIFIED: prior art / nguồn | INTERPRETATION / HYPOTHESIS: ảnh hưởng tới câu hỏi |
| --- | --- |
| [A audit](deep_dives/FAMILY_A_FORECASTING_AUDIT.md) đối chiếu manifold/PCA, dense deformation, probabilistic 4D prediction, Kalman/GP, online adaptation và cine forecasting. Gunnarsson MICCAI 2024 có cả online assimilation và forecast suffix; Pohl dùng direct horizon-specific prediction. | Không claim medical motion thiếu geometry, uncertainty hoặc forecast. Q1 phải đo giá trị bổ sung của state ở cùng prefix và endpoint; Q2 cần calibration/decision ranking ngoài variance. |
| [B audit](deep_dives/FAMILY_B_ACQUISITION_AUDIT.md) tìm thấy sequence memory, protocol discrimination, nonrigid reconstruction và online adaptation trước EchoWorld. TUS-REC loại invalid transforms trong preprocessing. | Q4 cần tách anatomy memory khỏi coverage/protocol; số frame lưu lại không tự là Δt. Full-sweep reconstruction không thể làm state prefix. Command/contact chưa được xác minh. |
| [C audit](deep_dives/FAMILY_C_LONGITUDINAL_AUDIT.md) tách direct target-time queries, patient adaptation, oracle sample selection, image-only external evaluation và mechanistic forward simulation. | Hai future visits từ cùng cutoff là protocol cần chấm, không phải novelty tự thân. Direct multi-time forecast vẫn hợp lệ; không bắt buộc recursive để được gọi world model. |
| [Metadata aggregate](../research_artifacts/2026-09-12_metadata_audit.json) tái lập 62 LUMIERE patients có ≥4 nominal weeks với đủ bốn MRI sequences. API ghi CC0 nhưng README non-commercial. | Có tín hiệu temporal depth để điều tra Q5; usable masks/intervals/access terms chưa đóng. Không tăng ranking thành “đã có data”. |
| [State/evaluation audit](deep_dives/STATE_AND_EVALUATION_AUDIT.md) tìm thấy conformal surgical forecasting, medical uncertainty recalibration và trajectory-level conformal methods; CV benchmarks cũng dùng nhiều estimated/judged proxies. | Không dùng “calibration đầu tiên trong medical” hoặc mặc định general-CV score đã đo physical truth. Chuyển giao tốt nhất là cách thử và bác bỏ state/rollout, có ground truth phù hợp. |

**Định nghĩa vận hành — INTERPRETATION / HYPOTHESIS.** Medical Visual World Model là mô hình suy một predictive state/belief có thể kiểm tra từ ảnh y khoa và thông tin thực có trước cutoff, định nghĩa cách state đổi theo thời gian và các biến điều kiện được phép, rồi dự báo state/observation tương lai ở horizon rõ ràng để phục vụ một mục đích có thể đánh giá. Cam kết thêm của tên gọi là state có giá trị dự báo/tái sử dụng được kiểm tra, temporal contract minh bạch và evaluation vượt qua tái tạo/perceptual plausibility. Không đòi state là sinh học, action, policy, pixel output hoặc một họ kiến trúc; simulation/planning/causal/clinical claims cần bằng chứng riêng.

**Map chuyển giao — INTERPRETATION / HYPOTHESIS:** PSR/representation world models → kiểm tra information sufficiency/reuse → Q1/Q4/Q6; object/geometry dynamics → future structure error → Q1/Q3/Q5; functional CV benchmarks → đánh giá qua target/use case độc lập → cả A/B/C; belief/stochastic dynamics → calibration và joint futures có proper scores → Q2/Q5/Q7; embodied active acquisition → coverage/support theo budget → Q4 nếu có dữ liệu phù hợp. Medical prior art đã có nhiều thành phần này, nên map là cơ hội kiểm chứng capability trong một temporal contract cụ thể, không là danh sách novelty. Nguồn cụ thể và phản chứng ở [TRANSFER_GAPS](TRANSFER_GAPS.md) và [state audit](deep_dives/STATE_AND_EVALUATION_AUDIT.md).
